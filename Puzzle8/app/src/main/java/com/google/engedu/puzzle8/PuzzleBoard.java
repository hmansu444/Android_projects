...
public class PuzzleBoard {
        ...
        for (int y = 0; y < NUM_TILES; y++) {
            for (int x = 0; x < NUM_TILES; x++) {
                ...
                if (tileNumber != NUM_TILES * NUM_TILES - 1) {
                    ...
                    //int tileSize=scaledBitmap.getWidth();
                    Bitmap tileBitmap = Bitmap.createBitmap(scaledBitmap, x * scaledBitmap.getWidth() / NUM_TILES, y * scaledBitmap.getWidth() / NUM_TILES,
                            parentWidth / NUM_TILES, parentWidth / NUM_TILES);
    ...
    public ArrayList<PuzzleBoard> neighbours() {
        ...
        for (int[] coordinates : NEIGHBOUR_COORDS) {
            ...
            if (neighbourX >= 0 && neighbourX < NUM_TILES && neighbourY >= 0 && neighbourY < NUM_TILES) {
                ...
                neighbourBoard.swapTiles(XYtoIndex(neighbourX, neighbourY),
    ...
    public int priority() {
        ...
        for (int i = 0; i < NUM_TILES * NUM_TILES; i++) {
            PuzzleTile tile = tiles.get(i);
            //int correntPosition = tile.getNumber();
            //String c="";
            //Log.e(c,"currentPosition"+correntPosition);
            if (tile != null) {
                int correctPosition;
                correctPosition= tile.getNumber();
